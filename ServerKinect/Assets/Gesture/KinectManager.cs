...
public class KinectManager : MonoBehaviour
{
    ...
    public Text GestureThrowProgTextGameObject;
    public Text ConfidenceThrowProgTextGameObject;

    public Text GestureShootTextGameObject;
    public Text ConfidenceShootTextGameObject;

    public Text GestureShootProgTextGameObject;
    public Text ConfidenceShootProgTextGameObject;
    ...
    private string throwGesture = "throw2";
    private string throwProgress = "throw2Progress";
    private string shootGesture = "shoot";
    private string shootProgress = "shootProgress";
    ...
    void Start()
    {
        ...
        gestureDetectorList.Add(new GestureDetector(_BodyManager.GetSensor()));
        ...
        if (this.kinectSensor != null)
        {
            ...
            for (int bodyIndex = 0; bodyIndex < this.bodyCount; bodyIndex++)
            {
                ...
                GestureThrowProgTextGameObject.text = "none";
                GestureShootTextGameObject.text = "none";
    ...
    private void OnGestureDetected(object sender, GestureEventArgs e, int bodyIndex)
    {
        ...
        if(e.GestureID == throwGesture)
        {
            //NEW UI FOR GESTURE DETECTed
            GestureTextGameObject.text = "Gesture Throw Detected: " + isDetected;
            //StringBuilder text = new StringBuilder(string.Format("Gesture Detected? {0}\n", isDetected));
            ConfidenceTextGameObject.text = "Confidence Throw: " + e.DetectionConfidence;
        ...
        if (e.GestureID == throwProgress)
        {
            //NEW UI FOR GESTURE DETECTed
            GestureThrowProgTextGameObject.text = "ThrowProgress: " + progressGesture;
            //StringBuilder text = new StringBuilder(string.Format("Gesture Detected? {0}\n", isDetected));
            ConfidenceThrowProgTextGameObject.text = "Confidence ThrowProgress: " + e.DetectionConfidence;
            //text.Append(string.Format("Confidence: {0}\n", e.DetectionConfidence));
            /*if (e.DetectionConfidence > 0.65f)
            {
                turnScript.turnLeft = true;
            ...
            {
                ConfidenceThrowProgTextGameObject.text = "True";
          
            }
            else
            {
                ConfidenceThrowProgTextGameObject.text = "False";

            }
        }

        if (e.GestureID == shootGesture)
        {
            //NEW UI FOR GESTURE DETECTed
            GestureShootTextGameObject.text = "Gesture Shoot Detected: " + isDetected;
            //StringBuilder text = new StringBuilder(string.Format("Gesture Detected? {0}\n", isDetected));
            ConfidenceShootTextGameObject.text = "Confidence Shoot: " + e.DetectionConfidence;
            //text.Append(string.Format("Confidence: {0}\n", e.DetectionConfidence));
            /*if (e.DetectionConfidence > 0.65f)
            {
                turnScript.turnLeft = true;
        ...
        if (e.GestureID == shootProgress)
        {
            //NEW UI FOR GESTURE DETECTed
            GestureShootProgTextGameObject.text = "Shoot Progress: " + progressGesture;
            //StringBuilder text = new StringBuilder(string.Format("Gesture Detected? {0}\n", isDetected));
            ConfidenceShootProgTextGameObject.text = "Confidence ShootProgress: " + e.DetectionConfidence;
            //text.Append(string.Format("Confidence: {0}\n", e.DetectionConfidence));
            if (progressGesture > 0.9f)
            {
               //messagesClient.SendMessage("shoot");
            }
        }
