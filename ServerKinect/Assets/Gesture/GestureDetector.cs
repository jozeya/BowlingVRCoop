...
public class GestureEventArgs : EventArgs
{
    ...
    public float progressGesture { get; private set; }
    ...
    public GestureEventArgs(bool isBodyTrackingIdValid, bool isGestureDetected, float detectionConfidence, string gestureID)
    {
        ...
        this.IsGestureDetected = isGestureDetected;
        this.DetectionConfidence = detectionConfidence;
        this.progressGesture = 0;
        this.GestureID = gestureID; 
    }

    public GestureEventArgs(bool isBodyTrackingIdValid, float progressGesture, string gestureID)
    {
        this.IsBodyTrackingIdValid = isBodyTrackingIdValid;
        this.IsGestureDetected = false;
        this.DetectionConfidence = 0;
        this.progressGesture = progressGesture;
        this.GestureID = gestureID;
    }
}
...
public class GestureDetector : IDisposable
{
    ...
    private readonly string leanDB = "GestureDB\\bowlingGesture2.gbd";
    ...
    private readonly string throwGestureName = "throw2";
    private readonly string throwProgressName = "throw2Progress";
    private readonly string jumpGestureName = "jump";
    //private readonly string shootGestureName = "shoot";
    //private readonly string shootProgressName = "shootProgress";
    ...
    public GestureDetector(KinectSensor kinectSensor)
    {
        ...
        using (VisualGestureBuilderDatabase database = VisualGestureBuilderDatabase.Create(databasePath))
        {
            ...
            foreach (Gesture gesture in database.AvailableGestures)
            {
                ...
                if (gesture.Name.Equals(this.throwGestureName))
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }
                if (gesture.Name.Equals(this.throwProgressName))
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }
                ...
                /*if (gesture.Name.Equals(this.shootGestureName))
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }
                if (gesture.Name.Equals(this.shootProgressName))
                {
                    this.vgbFrameSource.AddGesture(gesture);
                }*/
            }
            ...
            if (frame != null)
            {
                ...
                if (discreteResults != null)
                {
                    ...
                    foreach (Gesture gesture in this.vgbFrameSource.Gestures)
                    {
                        ...
                        if (gesture.Name.Equals(this.throwProgressName) && gesture.GestureType == GestureType.Continuous)
                        {
                            ...
                            if (result != null)
                            {
                                if (this.OnGestureDetected != null)
                                {
                                    this.OnGestureDetected(this, new GestureEventArgs(true, result.Progress, this.throwProgressName));
                                    
                                }
                            }
                        }
                        /*
                        if (gesture.Name.Equals(this.shootGestureName) && gesture.GestureType == GestureType.Discrete)
                        {
                            DiscreteGestureResult result = null;
                            discreteResults.TryGetValue(gesture, out result);
                            ...
                            if (result != null)
                            {
                                if (this.OnGestureDetected != null)
                                {
                                    this.OnGestureDetected(this, new GestureEventArgs(true, result.Detected, result.Confidence, this.shootGestureName));
                                }
                            }
                        }

                        if (gesture.Name.Equals(this.shootProgressName) && gesture.GestureType == GestureType.Continuous)
                        {
                            //DiscreteGestureResult result = null;
                            ContinuousGestureResult result = null;
                            ...
                            if (result != null)
                            {
                                if (this.OnGestureDetected != null)
                                {
                                    this.OnGestureDetected(this, new GestureEventArgs(true, result.Progress, this.shootProgressName));
                                }
                            }
                        }*/
                    }
